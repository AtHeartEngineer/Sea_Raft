...
fn sync_starship_rotation_transform(mut query: Query<(&Starship, &mut Transform)>) {
...
fn remove_bullet(mut commands: Commands, query: Query<(Entity, &Bullet, &Position)>) {
...
fn decelerate_starship(keys: Res<Input<KeyCode>>, mut query: Query<&mut Velocity, With<Starship>>) {
